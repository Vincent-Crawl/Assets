...
class PreNorm(nn.Module):#预归一化处理和非线性变换
...
class FeedForward(nn.Module):#Iterative Shrinkage-Thresholding Algorithm优化
    ...
    def forward(self, x):
        ...
        output = F.relu(x + grad_update)#通过 ReLU 函数执行非线性变换。
        return output


#Attention:输入转换为 Q（查询）、K（键）和 V（值）三个矩阵
#通过计算 Q 和 K 的点积并进行 softmax 归一化
#注意力权重应用于 V 得到输出
...
import torch.nn.functional as F
class Transformer(nn.Module):# Transformer 编码器，由多个 Transformer 层组成
