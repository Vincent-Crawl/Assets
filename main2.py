...
args = {
    'data': "D:\桌面\CRATE-main\代码测试\data\cifar_new",
    'arch': 'vit_tiny',
    'workers': 8,
    'epochs': 90,
    'start_epoch': 0,
...
def main(args):
    ...
    if args.gpu is not None:
        warnings.warn('You have chosen a specific GPU. This will completely '
                      'disable data parallelism.')  # 这样的选择可能会限制训练的速度，因为无法利用到其他的GPU资源。

    if args.dist_url == "env://" and args.world_size == -1:
        args.world_size = int(os.environ["WORLD_SIZE"])
    # 检查是否使用环境变量来设置分布式训练的初始化

    args.distributed = args.world_size > 1 or args.multiprocessing_distributed  # 是否进行分布式训练
    ...
    if args.multiprocessing_distributed:
        ...
        args.world_size = ngpus_per_node * args.world_size  # 全局的进程数量（world size）应该是每个节点上的GPU数量和节点数量的乘积。
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        print(f'Accuracy at epoch {epoch}: {acc1}')
        scheduler.step()
